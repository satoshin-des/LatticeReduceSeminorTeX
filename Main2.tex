...
\begin{frame}
    ...
    \begin{definition}[specific~dimension]
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \label{alg_REDUCE}
        \Require{基底行列$\bm{B}\in M_{n}(\mathbb{Z})$，$0<\eta\le 1$}
...
\begin{algorithm}[H]
    ...
    \begin{algorithmic}[1]
        ...
        \label{alg_L2_2}
        \Require{$B, \bm{U}, \eta, \overline{\beta}$}
        \Ensure{簡約された基底$\{\bm{b}_1,\ldots,\bm{b}_n\}$}
        \State \textbf{if}~$n=2$~\textbf{then}~Schonhageのアルゴリズム\cite{Schonhage91}~\textbf{endif}
        \State $\bm{V}\gets \bm{E}_n$
        \For{$c=1$~to$(2n_r/n_{r-1})^2\lceil \log{(8\overline{\beta}/\eta)}\rceil$}
